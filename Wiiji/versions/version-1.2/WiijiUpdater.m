...
+ (void)doUpdate:(id)sender 
{
   ...
   if (!verStr) {
		...
		if ([verStr floatValue] > [myVer floatValue]) {
			if ([sender class] != [wiimote_manager class])
			...
			if ([sender class] != [wiimote_manager class]) // only show the dialog if the sender was not a programed response (only show if we clicked the menu)
				ret = NSRunAlertPanel(@"No Update Available", @"No update to Wiiji is available.\nYou have the current version.", @"OK", nil, nil);
		}
		if (ret == -1) {
			if ([sender respondsToSelector:@selector(setAutoUpdate:)]) {
				wiimote_manager* sent_from = sender;
