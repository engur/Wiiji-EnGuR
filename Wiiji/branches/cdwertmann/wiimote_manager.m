 ...
 * Note: HID bindings of the wii classic controller and the wii remote overlap wherever possible.  This should be changed to an optional user setting.
...
/* Project todo list:
 TODO: "check for update" support (sparkle 1.1 was removed last-minute)
 ** The sparkle 2.0 code supposedly can deal with non .app files.  perhaps we should just wait for that and use it on .kext or .mpkg/.pkg
 I was originally using sparkle for update management, but decided to use packagemaker to simplify the kext install process.
 Sparkle cant use pkg files, it needs app files, so I stopped using it.
 But, I want to drop packagemanager to make things easy, but this requires a few things:
 1 make an authentication object that gets the permissions needed to write the kext files, and call kextload/unload
   migrate our scripts over from the packagemaker distribution
 2 program needs to be able to check version of the kext
 3 program needs to disconnect wii motes, disconnect all kext referenes, and unload kext (will we have to quit too?)
 4 copy new kext over to extensions folder, load the new kext (but manually loaded kexts dont stick)
 * it should be easy but lets do that later if this program turns out to be popular

 TODO: STRUCTURE
 1 diversify into separate objects: WiiRemoteController, rename this object to AppController, ... , ...
 ...
 5 Real help guide
 6 wiimotes dont always connect (framework)
 ...
 13 auto Calibration of max min and center (framework)
...
#define desiredVirtualHIDVersion	@"1.0.0"
...
-(void)awakeFromNib
{
	...
	// todo: load version information from plist/bundle and set the string in the about box
...
- (void) pressureChanged:(WiiPressureSensorType) type pressureTR:(unsigned short) bPressureTR pressureBR:(unsigned short) bPressureBR 
	...
	if (totalWeight > 0) {
		...
		y = (((bPressureTR + bPressureTL) - (bPressureBR + bPressureBL)) / totalWeight) * -127;
	} else {
		x = 0;
		y = 0;
	}
