...
- (void) handleExtensionData:(unsigned char *) dp length:(size_t) dataLength
{
	...
	switch (expType) {
		case WiiNunchuk:
			...
			if ([_delegate respondsToSelector:@selector (accelerationChanged:accX:accY:accZ:controllerID:)])
				[_delegate accelerationChanged:WiiNunchukAccelerationSensor accX:nAccX accY:nAccY accZ:nAccZ controllerID:controller_id];
		...
		case WiiBalanceBoard:
			...
			if ([_delegate respondsToSelector:@selector (pressureChanged:pressureTR:pressureBR:pressureTL:pressureBL:controllerID:)])
				...
				[_delegate pressureChanged:WiiBalanceBoardPressureSensor pressureTR:bKg.tr pressureBR:bKg.br pressureTL:bKg.tl pressureBL:bKg.bl controllerID:controller_id];
...
- (void) handleButtonReport:(unsigned char *) dp length:(size_t) dataLength
{
	...
	if (dp[1] & 0x01) {
		...
		[_delegate accelerationChanged:WiiRemoteAccelerationSensor accX:accX accY:accY accZ:accZ controllerID:controller_id];
