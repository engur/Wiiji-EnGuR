...
//#define myReportSize 8				// size of actual report in bytes
#define myReportSize 10				// size of actual report in bytes
#define myReportDescSize 65		// size of following report descriptor in bytes
...
//	0x81, 0x02,                    //     INPUT (Data,Var,Abs) 
	...
	// buttons
	// need 28 (1C, 3.5 bytes)   if we separate wii remote and classic, and use dpads as buttons
	// need 11 (0B, 1.375 bytes) if we overlap wii remote and classic, and use dpad as XY
	...
	0x95, 0x1C,                    //   REPORT_COUNT (28)						// 4 bytes
...
// On second thought... this is somewhat convinient... but the list of cases to load is getting too long...
IOService *com_veltrop_taylor_driver_virtualhid::probe(IOService *provider, SInt32 *score)
{
	...
	IOLog("VirtualHID: driver node children: %d \n", total_kids);		// log this for user support!
	// 3 if the device is fresh, 9 if wiiji already grabbed it, X? if os-x tried to claim it in the hid device nodes but can't
	// 4 if osculator driver grabbed one of the L2Cap kids but osculator.app not open, 12 if osculator.app is open, 10 for os-x going crazy and osculator
	if (total_kids == 3 || total_kids == 9 || total_kids == 4 || total_kids == 12 || total_kids == 10)		
		return res;
...
OSString* com_veltrop_taylor_driver_virtualhid::newManufacturerString() const
{
	...
	return OSString::withCString("Nintendo");
}
	...
	if (data) {
		...
		if (_myReportBufferDescInternal) {
			...
			if (target == hid_XYZ) {
				...
				_myReportBufferDescInternal[2] = button_vector[3];
			}
			else if (target == hid_rXYZ) {
				_myReportBufferDescInternal[3] = button_vector[1];
				_myReportBufferDescInternal[4] = button_vector[2];
				_myReportBufferDescInternal[5] = button_vector[3];
			}
